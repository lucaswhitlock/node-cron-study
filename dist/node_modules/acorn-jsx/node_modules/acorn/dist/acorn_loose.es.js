...
lp.readToken = function () {
  ...
  for (;;) {
    try {
      ...
      if (replace) {
        if (this$1.options.locations) replace.loc = new SourceLocation(this$1.toks, getLineInfo(this$1.input, replace.start), getLineInfo(this$1.input, replace.end));
        return replace;
      }
    }
  }
...
lp$1.parseStatement = function () {
  ...
  switch (starttype) {
    ...
    case tokTypes._return:
      this.next();
      if (this.eat(tokTypes.semi) || this.canInsertSemicolon()) node.argument = null;else {
...
lp$1.parseClass = function (isStatement) {
  ...
  var node = this.startNode();
  this.next();
  if (this.tok.type === tokTypes.name) node.id = this.parseIdent();else if (isStatement) node.id = this.dummyIdent();else node.id = null;
  ...
  while (!this.closes(tokTypes.braceR, indent, line)) {
    ...
    if (isDummy(method.key)) {
      if (isDummy(this$1.parseMaybeAssign())) this$1.next();this$1.eat(tokTypes.comma);continue;
    }
...
lp$1.parseFunction = function (node, isStatement) {
  ...
  if (this.tok.type === tokTypes.name) node.id = this.parseIdent();else if (isStatement) node.id = this.dummyIdent();
...
lp$2.parseObj = function () {
  ...
  while (!this.closes(tokTypes.braceR, indent, line)) {
    ...
    } else if (this$1.options.ecmaVersion >= 5 && prop.key.type === "Identifier" && !prop.computed && (prop.key.name === "get" || prop.key.name === "set") && this$1.tok.type != tokTypes.comma && this$1.tok.type != tokTypes.braceR) {
